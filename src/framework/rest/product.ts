...
export function useProducts(options?: Partial<ProductQueryOptions>) {
  ...
  const {
      ...
      client.products.all(Object.assign({}, queryKey[1], pageParam)),
    {
      ...
      // Keep the current cards on screen (dimmed by the caller) while a filter/
      // city/sort change refetches, instead of blanking the grid to skeletons —
      // removes the "laggy, nothing happening" feel on filter changes.
