...
type TopAuthorsGridProps = {
  title?: string;
};

const TopAuthorsGrid: React.FC<TopAuthorsGridProps> = ({ title }) => {
  ...
  if (isLoading && !authors.length) {
    return (
      <SectionBlock title={title} href={Routes?.authors}>
    ...
    <SectionBlock title={title} href={Routes?.authors}>
      {!isLoading && !authors?.length ? (
        <div className="min-h-full px-9 pt-6 pb-8 lg:p-8">
          <NotFound text="text-no-authors" className="h-96" />
        </div>
      ) : (
        <div>
